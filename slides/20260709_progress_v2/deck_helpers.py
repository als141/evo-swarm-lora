...
BASE_D  = RGBColor(0x8A, 0x93, 0x9E)  # ベース単体（図と対応・視認性up）
SC9_D   = RGBColor(0x54, 0x61, 0x70)  # SC@9
...
# 源暎エムゴv2（5ウェイト）。太さはフォント名で表現し、合成ボールドは使わない。
FONT_BLACK = "GenEi M Gothic v2 Black"
FONT_HEAVY = "GenEi M Gothic v2 Heavy"
FONT_BOLD = "GenEi M Gothic v2 Bold"
FONT_MED = "GenEi M Gothic v2 Medium"
FONT_REG = "GenEi M Gothic v2 Regular"
# 後方互換エイリアス（v1呼び出しをv2の適切なウェイトへ写像）
FONT = FONT_REG
FONT_L = FONT_REG      # Lightは無い→Regular
FONT_M = FONT_MED
FONT_SB = FONT_BOLD    # SemiBoldは無い→Bold

# bold=True のときに昇格させるウェイト写像（合成boldを避ける）
_BOLD_UP = {FONT_REG: FONT_HEAVY, FONT_MED: FONT_HEAVY, FONT_L: FONT_HEAVY}
...
def _apply_run(run, fnt, opts):
    """boldはウェイト名で表現（合成bold禁止）。italicのみ font.italic を使う。"""
    if opts.get("bold"):
        fnt = _BOLD_UP.get(fnt, fnt)
    run.font.bold = False
    if opts.get("italic"):
        run.font.italic = True
    _set_run_font(run, fnt)
...
def bullets(slide, x, y, w, h, items, size=17, gap=10, color=INK,
    ...
    for i, it in enumerate(items):
        ...
        _apply_run(rm, FONT_BOLD, {})
        r = p.add_run(); r.text = txt
        r.font.size = Pt(opts.get("size", size))
        r.font.color.rgb = opts.get("color", color)
        _apply_run(r, opts.get("font", FONT), opts)
    return tb
...
def chip(slide, x, y, w, label, fill, txt_color=CARD, size=11, h=0.34):
    ...
    r.font.size = Pt(size); r.font.color.rgb = txt_color
    _apply_run(r, FONT_BOLD, {})
...
def section_divider(prs, num, title, sub=""):
    ...
    text(s, 1.2, 2.75, 1.5, 1.0, [(f"{num:02d}", 44, ACCENT, FONT_BLACK, {})])
    text(s, 1.25, 3.55, 10.5, 1.2, [(title, 33, CARD, FONT_BLACK, {})])
    if sub:
        text(s, 1.3, 4.5, 10.3, 0.9, [(sub, 15, RGBColor(0xC7, 0xCE, 0xD4), FONT_REG, {})])
    return s
