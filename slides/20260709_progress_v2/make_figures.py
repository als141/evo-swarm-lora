...
# ---- フォント登録（源暎エムゴv2）----
GENEI = "/home/als0028/.local/share/fonts/genei-mgothic2"
_names = {}
for f in Path(GENEI).glob("*.ttf"):
    fm.fontManager.addfont(str(f))
    _names[f.stem.split("-")[-1]] = fm.FontProperties(fname=str(f)).get_name()
plt.rcParams["font.family"] = _names.get("Medium", "GenEi M Gothic v2 Medium")
plt.rcParams["axes.unicode_minus"] = False
plt.rcParams["svg.fonttype"] = "none"
# ウェイト別 FontProperties（図中の強調に使用）
FP_REG = fm.FontProperties(fname=str(Path(GENEI) / "GenEiMGothic2-Regular.ttf"))
FP_MED = fm.FontProperties(fname=str(Path(GENEI) / "GenEiMGothic2-Medium.ttf"))
FP_BOLD = fm.FontProperties(fname=str(Path(GENEI) / "GenEiMGothic2-Bold.ttf"))
FP_HEAVY = fm.FontProperties(fname=str(Path(GENEI) / "GenEiMGothic2-Heavy.ttf"))
...
def fig_final():
    ...
    data = {  # %
        ...
        "LoRAチーム":        ([71.6, 86.7, 43.1], TEAM_N),
    }
    fig, ax = plt.subplots(figsize=(9.6, 4.9))
    x = np.arange(len(benches)); w = 0.24
...
def fig_verdict():
    rows = [  # (ラベル, 差, 判定, 色)
        ("多数決9回（SC@9）に対して（総合）", -3.0, "負け  ( p<0.001 )", NEG),
        ("ベース単体に対して（総合）", +0.3, "互角  ( 差なし )", SUB),
        ("多数決9回（SC@9）に対して（数学だけ）", -0.7, "互角  ( 差なし )", POS),
    ]
    ...
    for yi, (lab, d, tag, col) in zip(y, rows):
        ...
        ax.text(d + (0.18 if d >= 0 else -0.18), yi, f"{d:+.1f}pt",
                ...
                fontsize=13, color=col, fontproperties=FP_BOLD)
    ...
    ax.set_xlabel("LoRAチームの正答率の差（ポイント）", fontsize=10)
    for sp in ["top", "right", "left"]:
        ax.spines[sp].set_visible(False)
    ax.spines["bottom"].set_color(INK)
    ax.tick_params(length=0)
    fig.subplots_adjust(left=0.34, right=0.80)
...
# ================================================================
# 図6: 進化が動かしたもの（＝数学の"壊れ"の部分修復のみ）
# ================================================================
def fig_evolution():
    labs = ["一般知識・推論\n(MMLU-Pro)", "数学\n(MATH-500)", "大学院級科学\n(SuperGPQA)"]
    deltas = [-0.1, +4.3, -0.8]  # 進化後 − 進化前（実験1・同一環境）
    fig, ax = plt.subplots(figsize=(8.6, 4.5))
    x = np.arange(len(labs))
    cols = [BASE if abs(d) < 1 else POS for d in deltas]
    bars = ax.bar(x, deltas, 0.5, color=cols, zorder=3,
                  edgecolor="white", linewidth=1)
    for b, d in zip(bars, deltas):
        ax.text(b.get_x()+b.get_width()/2, d + (0.2 if d >= 0 else -0.2),
                f"{d:+.1f}pt", ha="center", va="bottom" if d >= 0 else "top",
                fontsize=12, color=(POS if abs(d) >= 1 else SUB),
                fontproperties=FP_BOLD)
    ax.axhline(0, color=INK, linewidth=1.2, zorder=2)
    ax.set_xticks(x); ax.set_xticklabels(labs, fontsize=11)
    ax.set_ylabel("進化による正答率の変化（pt）", fontsize=10.5)
    ax.set_ylim(-2.4, 6)
    ax.annotate("動いたのは数学だけ\n（強化ではなく“壊れ”の部分修復）",
                xy=(1, 4.3), xytext=(1.35, 5.3),
                fontsize=10, color=POS, fontproperties=FP_BOLD,
                ha="left", va="center")
    ax.text(0, -2.0, "ほぼ変化なし", ha="center", fontsize=9.5, color=SUB)
    ax.text(2, -2.0, "ほぼ変化なし", ha="center", fontsize=9.5, color=SUB)
    _style(ax)
    ax.yaxis.grid(True, color=GRID, linewidth=0.9, zorder=0)
    ax.set_axisbelow(True)
