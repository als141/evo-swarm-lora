...
judge（審判）エージェントによる集約を採用しない根拠は二つある．第一に，MADの性能利得の大半は議論そのものではなく多数決集約で説明されるという分析\cite{choi2025debate}，および推論タスクでは合意形成プロトコルより投票が一貫して優れるという比較実験\cite{kaesberg2025voting}があり，集約規則としての多数決の優位が示されている．第二に，議論の最終決定を担うエージェントの判定能力がチーム性能を左右する（意思決定者がボトルネックとなる）ことが報告されており\cite{judge2025bottleneck}，本研究のように4B級の小型モデルのみでチームを構成する設定では特に深刻である．また，言語化された確信度（confidence）は過信バイアスを持つため主集約には用いない．
...
また，式(\ref{eq:shapley3})の第1項が示すとおり$\phi_c$は$v(\{c\})$（solo精度）を重み$\frac{1}{3}$で内包する．したがって個体タスク性能を別項として適応度に加算する必要はなく，「個体性能＋協調寄与」が単一のスカラーに公理的に統合される．これが個体性能のみを適応度とする先行研究（GENOME\cite{zhang2025genome}）との本質的な差分であり，この差分はアブレーションA1（適応度を$v(\{c\})$に置換）として直接検証可能な設計とした（実施状況は\S\ref{sec:4.3}）．
...
\textbf{重要な注意（実装構成による選抜中立性）}: 本実験の役割内サブ集団は$K=2$である（\S\ref{sec:3.4}）．このとき式(\ref{eq:sharing})の和は相手1個体との距離のみからなり，$d(a,b)=d(b,a)$の対称性から\textbf{2個体のsharing係数は常に同値}になる．乗法ペナルティは正の定数倍として順位を保存するため，役割内選抜（$\arg\max_c F(c)$）はShapley値のみの選抜と完全に一致する．すなわち\textbf{fitness sharingは$K=2$の本構成では選抜に作用しない}．この帰結は執筆後の検証で判明したものであり，実際の進化ログにおける事後検証（全18ケースで係数が役割内同値・選抜順位不変）を\S\ref{sec:4.6.6}に示す．sharingを設計意図どおり機能させるには$K \geq 3$が必要であり，本章の記述は「導入した設計」として残すが，実験結果の解釈においてsharingの効果を主張することはできない．
...
そこで本研究は，（i）選択圧そのものはチーム貢献$\phi_c$に一元化し，（ii）多様性はfitness sharingの\textbf{乗法ペナルティ}として「同一nicheへの密集」を抑制する形でのみ作用させる，という分離設計を採る．乗法形式は性能と多様性の単位の混合を避け，$\phi_c$の順位構造をniche内でのみ再調整する．ただし前項末尾に述べたとおり，この設計は$K=2$の本実験構成では選抜中立であり（アブレーションA2のsharing無効化も同構成では自明に無差になる），多様性維持機構としての実効性の検証は$K \geq 3$での再実験に委ねられる．
...
\label{eq:selection}
\end{equation}
選ばれた代表は\textbf{エリート保存}により無変更のまま次世代サブ集団に残る．これは突然変異による劣化がチーム全体へ伝播することを防ぐための設計である．ただし候補の適応度は「他役割の\textbf{現}代表と組んだ文脈」で測られ，この評価文脈（他役割の代表とsharingの対象個体）は世代ごとに変わるため，エリート保存があっても代表系列の適応度の単調非減少は保証されない点に注意する（実測でも同一個体の測定チーム精度は世代間で$\pm 4$〜$7$pt変動した; \S\ref{sec:4.6.2}）．
...
これにより子の実効更新$B'A' = U_r \Sigma_r V_r^\top$は，補間された$\Delta W'$のFrobeniusノルム最良rank-$r$近似（Eckart--Youngの意味で，ランダム化SVDによる近似精度の範囲内）となり，交差項の混入と再パラメータ化依存性の両方が排除される．特異値の平方根を$A$，$B$に均等配分するのは，両行列のスケールを揃え後続の突然変異ノイズ（テンソルの標準偏差に対する相対スケール）が偏らないようにするためである．親同士のrankと対象モジュール集合の一致は実行時に検証する．naive方式は実装内に保持し，アブレーションA3として比較可能な形にした．
