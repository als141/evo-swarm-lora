...
\begin{itemize}
\item \textbf{シードと反復}: 主要比較は$K = 3$シード（1, 2, 3）で反復する．各（シード, 問題）対は両条件で同一の生成条件を共有するため，検定は（シード, 問題）を単位とする対応ありデータとして扱い，シードを結合したプールで行う．
\item \textbf{主検定}: 2値（正解/不正解）の対応ありデータに適した\textbf{McNemar正確検定}\cite{dror2018hitchhiker}を（シード, 問題）対に適用する．効果量として不一致対の内訳（一方のみ正解の問題数）と精度差を併記する．
...
\item \textbf{頑健性確認}: 実験2の主検定（条件7$^\ddagger$ vs SC@9）が3シードで境界的（$p=0.053$）となったため，\textbf{効果の維持を確認する目的を事前に明記した上で}seed 4--6を追加し，3シード単独と6シード合算の両方を報告する（optional stoppingの批判に対する透明性の確保）．
...
主要な検定結果（プール3{,}000問，McNemar正確検定，Holm補正）は以下のとおりである．

\begin{itemize}
\item \textbf{チーム化の効果（MMLU-Pro）}: LoRAペルソナチーム（条件4）はベース単体に対し$+4.5$pt（$p=0.0002$），素の議論に対し$+8.6$pt相当の有意な優位を示した．
\item \textbf{計算量マッチとの比較}: 進化後チーム（条件5）はSC@9に対しMMLU-Proで$+1.6$pt（非有意）だが，プール全体では$-2.2$pt（$p=0.004$）と\textbf{有意に劣った}．
\item \textbf{進化の寄与}: 条件5と条件4の差はMATH-500の$+4.3$pt（$p=0.0007$）のみ有意で，プール全体では$+0.6$pt（非有意）であった．
\item \textbf{領域依存性}: 素の議論はMATH-500で$+4.0$pt（$p<0.0001$）・SuperGPQAで$+3.1$pt（$p=0.022$）と有意に有効だが，MMLU-Proでは$-4.1$pt（$p=0.0009$）と有意に有害だった．
\end{itemize}

すなわち実験1の結論は「LoRAペルソナチームは知識推論（MMLU-Pro）でのみ有効であり，計算量を揃えたSelf-Consistencyには総合で及ばない」である．
...
\item[G1（再学習と個体能力の検収）] 既存ペルソナ例60例にリプレイ36例（約38\%）を混合し，rank 16・2エポック・学習率$1 \times 10^{-4}$・系列長8{,}192トークンで3ペルソナを再学習した（run002）．solo精度はMMLU-Pro 0.665--0.695，MATH-500 0.775--0.845，SuperGPQA 0.405--0.440となり，全ベンチマークで事前基準（0.63/0.75/0.37）を満たした．\textbf{能力毀損は完治し，一部はベース単体を上回った}．
...
主要な検定結果（McNemar正確検定，Holm補正）:
\begin{itemize}
\item \textbf{主検定（対SC@9，6シード・プール6{,}000問）}: 条件7$^\ddagger$は$-2.98$pt（勝ち223問，負け402問，$p<10^{-6}$）と\textbf{有意に劣った}．ベンチマーク別ではMMLU-Pro $-2.4$pt（$p<10^{-4}$），MATH-500 $-0.7$pt（$p=0.27$，同等），SuperGPQA $-5.4$pt（$p<10^{-5}$）である．
\item \textbf{処方の効果（対旧チーム，3シード・プール3{,}000問）}: 条件7$^\ddagger$は旧チームに対し$+3.20$pt（$p<10^{-5}$，Holm補正後有意）と\textbf{一貫して優った}（MMLU-Pro $+3.7$pt・MATH-500 $+6.3$pt・SuperGPQA $+0.2$pt）．診断に基づく処方（リプレイ再学習・条件付き議論・匿名化・重み付き投票）の効果は測定環境の統制後も頑健である．
\item \textbf{対ベース単体}: 条件7$^\ddagger$は$+0.27$pt（$p=0.71$）と統計的同等であり，実験1で確認された「ペルソナSFTチームの能力毀損」（旧チームはベースに$-2.93$pt，$p<10^{-4}$で有意に劣る）を\textbf{ベース水準まで完治}させた．ベンチマーク別ではMATH-500のみ$+3.8$pt（$p=0.0003$）と有意に上回る．
\end{itemize}

計算量の観点では条件7$^\ddagger$は1問あたり6生成とSC@9の9生成より少ないが，性能で相殺するには至らなかった．すなわち実験2の到達点は「処方によりチームを\textbf{ベース単体と同等（数学では有意に優位）}まで修復したが，計算量マッチのSelf-Consistencyには及ばない」である．
