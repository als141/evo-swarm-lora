...
- **新規分析スクリプト5本**（scripts/analysis/、全て手元データのみで再実行可能）:
  1. `verify_sharing_neutrality.py` — **査読指摘#1を実証**: K=2では sharing 係数が役割内で常に同値＝選抜に一切影響しない（構造的無効）。全18ケース（6世代×3役割）で確認、うち16ケースは距離がσ=0.3超で係数自体1.0（割引不発動）。**実際に働いた選択圧はShapley値のみ**。
  2. `final_stats_clustered.py` — **統計の全面再解析**: シードプールMcNemar（反復測定の相関を無視＝不適切）→ 問題IDクラスタの符号反転置換検定（20,000回）＋クラスタブートストラップCI（10,000回）＋TOST型同等性判定（±2ptマージン）へ。**主要結論は全て生存**: c7vsSC@9 = **−3.49pt [−4.50,−2.56] p<1e-4 有意負け**（推定対象が問題単位平均に変わるため点推定は−2.98→−3.49に変化）/ 処方効果 c7vsc5 = **+2.68pt [+1.23,+4.13] p=0.0003** / c7vsc1 = −0.32pt ns で **±2pt同等性成立**（TOST、90%CI[−1.47,+0.81]）/ **MATHのc7vsSC@9はΔ−0.78で差は検出されずだが90%CI下限−2.13で同等性未確定**（「肩を並べた」は言い過ぎ→修正）。実験1: c5vsSC@9 −1.84pt p=0.028（有意維持）/ 進化寄与 +0.18pt ns（MATHのみ+4.31pt p=0.0005）/ 領域依存3ベンチとも符号・有意性維持。MATH-500 L4-5の固有問題数は262（査読の指摘通り）。
  3. `mmlupro_category_analysis.py` — **検証可能性仮説のベンチ内検証→不支持**: MMLU-Pro内のmathカテゴリで素の議論は**−9.3pt (p=0.04) 悪化**（MATH-500では+4.2pt改善なのに）。定量系vs知識系のグループ差 p=0.61。→「検証可能性による統一説明」は仮説に格下げ、主張は「ベンチマーク依存の反転」に修正。
  4. `token_accounting.py` — **計算量の実測**（llm_calls全ログ×Qwenトークナイザ）: c7=6呼/問・入力9,925・出力5,879・計15,804tok/問 vs SC@9=9呼/問・入力1,955・出力16,464・計18,419tok/問。**出力（計算コスト支配項）はSC@9が2.8倍、総トークンでもSC@9が上**＝チームが計算量で優遇された比較ではない。「計算量マッチ」→「生成回数マッチ」に全文修正しつつ、比較の保守性は実測で担保。
  5. `delta_w_similarity.py` — **【最重要の新事実】進化は重みをほぼ動かしていなかった**: gen0 vs 進化後最終チームのΔWは全252モジュールで**cos=0.9996〜1.0001（実質同一）**、変異1回cos=0.9997・交叉1回cos=0.9998・4世代分cos=0.9998。**「ノルム保存・方向回転・損傷打ち消し（修復機構）」という旧解釈は棄却**。MATH+4.3ptは固定極小摂動への生成感度（環境系統差+6ptと同種）として説明するのが最も保守的。進化ループは「探索の不全(演算子摂動が極小)×多様性圧の不全(sharing中立)×選抜の不全(SE±4.9pt)」の**三重の機能不全**であり、チームレベル適応度の枠組み自体は支持も反証もされていない、が正確な結論。
- **修論修正**（コンパイル検証済み: エラー0・未解決参照0・main.pdf 567KB）: §4.4統計設計をクラスタ解析中心に書換＋TOST＋optional stopping正直記載＋再現性矛盾解消 / §4.6.1・4.6.5の全検定値差し替え / 表4.11の0.813→0.810修正（3シード平均の混入、200問で0.813は算術的に不可能という査読指摘が正解）＋タイブレーク脚注 / **§4.6.6新設**（sharing事後検証・カテゴリ別分析・トークン実測表）/ §3.3.2にK=2選抜中立の注記・§3.4.2エリート保存の単調性主張を撤回 / §5.1検証可能性を仮説に格下げ・§5.2「完治」「多様性維持」撤回＋再学習の交絡明記・§5.3進化解釈を三重機能不全に全面書換・§5.4 −3.5pt/同等性未確定に修正・§5.5限界3項目追加 / Sec1貢献の「役割間交叉」→「役割内」（実装と不一致だった）・sharing無効注記 / Sec6・abstract全面整合。
- **スライドv2の事実誤り修正**（発表7/9は終了済みのためアーカイブ修正）: 付録A1「ベースも6シード」→3シードに（査読指摘が正解）、「統計的に互角」→「差は検出されず・同等性未確定」、1/13の3進表記 0.00202…→0.002002…、表のはみ出し修正。**「進化=修復」スライド（発見②）は発表時のまま**（解釈変更は修論を正とし、スライド改変はユーザー判断に委ねる）。
- **査読指摘のうち反証できたもの**: (1)「実験結果の実在性は検証不能」→ per-item JSON・llm_calls完全ログ・アダプタ実体・ゲート事前定義のgit履歴が全てローカル＋GitHubに存在。(2) tinyBenchmarks誤適用 → 我々の引用は「ランダム100問はノイズ」批判側＋IRT厳選は今後の課題の文脈で概ね適正。(3) 暗号デモの翻訳問題 → v2で結婚問題に差し替え済み（査読はv1を見ていた）。
- **教訓**: (a) K=2のsharingが恒等変換になることは式を1行書けば分かった——設計時の「机上検算」の欠如。(b) ノルム比1.00で「方向回転」と解釈したのは検証しない飛躍——cosを測れば5分で棄却できた。(c) 負の結果の研究こそ、主張を支える統計・因果の作法が生命線。
